	...
	<form id="translation-form" method="POST" use:enhance class="mt-6 space-y-6">
		...
		<div class="grid gap-6 md:grid-cols-2">
			<div>
				...
				>
					{#each languages as lang}
						<option value={lang.code} selected={(form?.originalLang ?? 'ja') === lang.code}>{lang.name}</option>
					{/each}
				</select>
			</div>
			...
			<div>
				...
				>
					{#each languages as lang}
						<option value={lang.code} selected={(form?.translationLang ?? 'en') === lang.code}>{lang.name}</option>
					{/each}
				</select>
			</div>
		</div>
